...
import { Zap, Globe, Activity, FlaskConical, CheckCircle2, Database, Code2, Image, Type, Shield, Plug } from 'lucide-react';
import { Toggle, ListField, Card, PluginControlCard, getPath, type SiteContext } from './fields';

/* ------------------------------------------------------------------ */
/* Full site settings panel — grouped into optimization-category tabs */
/* ------------------------------------------------------------------ */
...
const TABS = [
  { id: 'cache', label: 'Page Cache', icon: Database },
  { id: 'css', label: 'CSS & Critical CSS', icon: Zap },
  { id: 'js', label: 'JavaScript', icon: Code2 },
  { id: 'media', label: 'Media & CDN', icon: Image },
  { id: 'fonts', label: 'Fonts & HTML', icon: Type },
  { id: 'safety', label: 'Dynamic & Safety', icon: Shield },
  { id: 'plugins', label: 'Plugin Assets', icon: Plug },
] as const;
...
export function SettingsPanel({ config, upsert, applyPreset, siteContext, className }: SettingsPanelProps) {
  ...
  const [tab, setTab] = useState<TabId>('cache');
  ...
  return (
    <div className={className}>
      ...
      {/* Category tabs */}
      <div className="mt-4 flex flex-wrap gap-1.5 rounded-2xl bg-white p-1.5 shadow-[0_1px_3px_rgba(23,23,23,0.06)]">
        {TABS.map((t) => {
          const Icon = t.icon;
          ...
          return (
            <button
              ...
              type="button"
              onClick={() => setTab(t.id)}
              className={`inline-flex items-center gap-1.5 rounded-xl px-3.5 py-2 text-[12.5px] font-semibold transition-all ${
                active ? 'bg-[#171717] text-white shadow-sm' : 'text-[#52525b] hover:bg-[#f4f4f5]'
              }`}
            >
