...
import { useSignIn, useSignUp, useClerk, SignIn, SignUp } from '@clerk/clerk-react';
import { isClerkAPIResponseError } from '@clerk/clerk-react/errors';
import { Mail, Lock, Eye, EyeOff, AlertCircle, ArrowLeft, CheckCircle2, Loader2, Sparkles, SlidersHorizontal } from 'lucide-react';
...
export const CustomAuth: React.FC<CustomAuthProps> = ({
  ...
  const [useClerkWidget, setUseClerkWidget] = useState(false);
  ...
  const handleSignInSubmit = async (e: React.FormEvent) => {
    ...
    if (!isSignInLoaded || !signIn) {
      setErrorMessage('Clerk authentication is still initializing. Please wait a moment.');
      return;
    }
    ...
    try {
      console.log('[Clerk SignIn] Submitting credentials for', email);
      const result = await signIn.create({
        identifier: email.trim(),
        password,
      });

      console.log('[Clerk SignIn] Status result:', result.status);
  ...
  const handleSignUpSubmit = async (e: React.FormEvent) => {
    ...
    if (!isSignUpLoaded || !signUp) {
      setErrorMessage('Clerk registration is still initializing. Please wait a moment.');
      return;
    }
    ...
    try {
      console.log('[Clerk SignUp] Registering email:', email);
      const result = await signUp.create({
        emailAddress: email.trim(),
        password,
      });

      console.log('[Clerk SignUp] Result status:', result.status, result);
  ...
  const handleVerifyEmail = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signup' && signUp) {
        ...
        console.log('[Clerk Verify Email] SignUp result:', result.status);
  ...
  return (
    <div className="w-full text-left">
      {/* Bot Detection Container for Clerk */}
      <div id="clerk-captcha" className="hidden" />

      {/* Embedded Clerk Widget Option (if toggled) */}
      {useClerkWidget ? (
        <div className="space-y-3 animate-fade-in">
          <div className="flex justify-between items-center pb-2 border-b border-[#f1f1f2]">
            <span className="text-xs font-semibold text-[#171717]">Clerk Fast Gateway</span>
            <button
              onClick={() => setUseClerkWidget(false)}
        ...
        <>
          ...
          {step === 'credentials' && (
            <div className="space-y-4">
              ...
              {/* Error Banner */}
              {errorMessage && (
                <div className="flex items-start gap-2 p-3 bg-[#fef2f2] border border-[#fecaca] rounded-xl text-xs text-[#dc2626] animate-shake">
                  <AlertCircle className="w-4 h-4 flex-none mt-0.5" />
                  <span>{errorMessage}</span>
                </div>
              )}
              ...
              <form onSubmit={mode === 'signin' ? handleSignInSubmit : handleSignUpSubmit} className="space-y-3">
                ...
                <button
                  type="submit"
                  disabled={isLoading || (mode === 'signin' ? !isSignInLoaded : !isSignUpLoaded)}
                  className="w-full btn btn-primary py-2.5 text-xs font-semibold flex items-center justify-center gap-2 mt-2 disabled:opacity-50"
                >
                  {isLoading ? (
              ...
              {/* Seamless Alternative Widget Switcher */}
              <div className="pt-2 text-center">
                <button
                  type="button"
                  onClick={() => setUseClerkWidget(true)}
                  className="text-[11px] text-[#71717a] hover:text-[#171717] flex items-center justify-center gap-1 mx-auto"
                >
                  <SlidersHorizontal className="w-3 h-3" />
                  <span>Use Clerk Fast Modal Gateway</span>
                </button>
              </div>
            </div>
          )}
