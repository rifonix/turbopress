...
class VideoFacade {
    ...
    private function build_youtube_facade(string $original_tag, string $src, string $video_id): string {
        ...
        return sprintf(
            '<div class="tp-video-facade tp-youtube-facade" data-src="%s" onclick="load_turbopress_video(this)" role="button" tabindex="0" aria-label="%s" style="background-image:url(\'%s\'),url(\'%s\')"><svg class="tp-play-btn" viewBox="0 0 68 48" width="68" height="48"><path class="tp-play-bg" d="M66.52,7.74c-0.78-2.93-2.49-5.41-5.42-6.19C55.79,.13,34,0,34,0S12.21,.13,6.9,1.55 C3.97,2.33,2.27,4.81,1.48,7.74C0.06,13.05,0,24,0,24s0.06,10.95,1.48,16.26c0.78,2.93,2.49,5.41,5.42,6.19 C12.21,47.87,34,48,34,48s21.79-0.13,27.1-1.55c2.93-0.78,4.64-3.26,5.42-6.19C67.94,34.95,68,24,68,24S67.94,13.05,66.52,7.74z" fill="#f00"></path><path d="M 45,24 27,14 27,34" fill="#fff"></path></svg></div>',
            esc_url($play_src),
            $title,
            esc_url($thumb_url),
    ...
    private function build_vimeo_facade(string $original_tag, string $src, string $video_id): string {
        ...
        return sprintf(
            '<div class="tp-video-facade tp-vimeo-facade" data-src="%s" onclick="load_turbopress_video(this)" role="button" tabindex="0" aria-label="%s"><svg class="tp-play-btn" viewBox="0 0 68 48" width="68" height="48"><rect width="68" height="48" rx="12" fill="#00adef"></rect><path d="M 45,24 27,14 27,34" fill="#fff"></path></svg></div>',
            esc_url($play_src),
            $title
        );
    }
    ...
    private function inject_facade_assets(string $html): string {
        $css = '<style id="tp-video-facade-css">.tp-video-facade{position:relative;display:block;width:100%;aspect-ratio:16/9;background-size:cover;background-position:center;cursor:pointer;overflow:hidden;background-color:#000}.tp-video-facade:hover .tp-play-btn{transform:scale(1.1)}.tp-video-facade iframe{width:100%!important;height:100%!important;position:absolute;top:0;left:0;border:0}.tp-play-btn{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);transition:transform .2s ease;filter:drop-shadow(0 4px 8px rgba(0,0,0,.4))}</style>';
        $js = '<script id="tp-video-facade-js">function load_turbopress_video(el){if(el.getAttribute("data-loaded"))return;el.setAttribute("data-loaded","1");var ifr=document.createElement("iframe");ifr.src=el.getAttribute("data-src");ifr.setAttribute("frameborder","0");ifr.setAttribute("allow","accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share");ifr.setAttribute("allowfullscreen","1");el.innerHTML="";el.appendChild(ifr);}</script>';
